...
class BaseDetector(object):
  ...
  def pre_process(self, image, scale, meta=None):
    ...
    if self.opt.fix_res:  #测试时是否改变图片分辨率
    ...
    trans_input = get_affine_transform(c, s, 0, [inp_width, inp_height])  #根据预定义的图片大小定义一个仿射变换
  ...
  def run(self, image_or_path_or_tensor, meta=None):
    ...
    if isinstance(image_or_path_or_tensor, np.ndarray):
      ...
      file_name = image_or_path_or_tensor.split('/')[-1]
    ...
    for scale in self.scales:  
      ...
      if not pre_processed:   #对输入图片进行简单的预处理（仿射变换）
        images, meta = self.pre_process(image, scale, meta)
      else:
        # import pdb; pdb.set_trace()
        images = pre_processed_images['images'][scale][0]   #实际上是控制输入图片为不同大小的分辨率分别进行检测，并将检测结果通过后处理进行整合；
